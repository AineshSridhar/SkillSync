...
import { cookies as nextCookies } from "next/headers"; // alias to be safe
import jwt from "jsonwebtoken";
import prisma from "@/lib/db";

export async function POST(request: Request): Promise<NextResponse> {
  // 1. Authenticate user
  const cookieStore = await Promise.resolve(nextCookies()); // force TS to see it as sync
  ...
  try {
    ...
    await prisma.profile.upsert({
      ...
      create: { userId, profilePic: blob.url },
    });
